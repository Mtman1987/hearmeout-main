...
2. The permanent SpaceMountain Lounge room is `system-spacemountainlive-lounge`. It has two separate permanent queues: `SPACEMOUNTAIN_LOUNGE_MUSIC_SESSION_ID` for `!sr` and `SPACEMOUNTAIN_LOUNGE_MOVIE_SESSION_ID` for `!wr`. The overlay may choose which queue to display/play, but the queues must never be merged.
...
7. During active Lounge development, the permanent Lounge media endpoints are intentionally allowed to run without user/service authorization so playback can be proven first. Reintroduce authorization only after the owner approves the working media path.
8. A future HMO replacement requires an explicit owner-approved atomic cutover. The new authority must replace Live HMO in one cutover; running both as competing authorities is forbidden.
