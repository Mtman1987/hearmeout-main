...
export async function GET(request: Request, context: { params: Promise<{ kind: string; streamId: string }> }) {
  ...
  try {
    if (kind === 'vod') {
      ...
      if (cached) {
        ...
        if (cachedResponse) {
          ...
          if (download) headers.set('content-disposition', `attachment; filename="xtream-${streamId}.mp4"`);
    ...
    if (download && kind === 'vod') headers.set('content-disposition', `attachment; filename="xtream-${streamId}.mp4"`);
