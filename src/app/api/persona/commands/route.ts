...
async function forwardToSpmt(accessToken: string, body: PersonaCommandBody) {
  const command = text(body.command || body.message || body.transcript, 5000);
  const personaId = text(body.personaId || body.persona?.id, 128);
  const personaName = text(body.personaName || body.persona?.name, 128);
  const voice = text(body.voice || body.persona?.voice, 128);
  const response = await fetch(`${SPMT_BASE_URL}/api/persona/commands`, {
    method: 'POST',
    headers: {
...
export async function POST(request: NextRequest) {
  const body = await request.json().catch(() => null) as PersonaCommandBody | null;
  const command = text(body?.command || body?.message || body?.transcript, 5000);
  if (!command) {
  ...
  if (!accessToken) {
    return NextResponse.json({ error: 'Sign in with SPMT to use bot personas' }, { status: 401 });
  }
