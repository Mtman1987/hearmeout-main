...
fly ssh sftp shell -a hearmeout-main -C "put youtube-cookies.txt /data/youtube-cookies.txt"

if %errorlevel% neq 0 (
    echo.
    echo Auto-upload failed. Try manually:
    echo   fly ssh console -a hearmeout-main -C "cat > /data/youtube-cookies.txt" < youtube-cookies.txt
    echo.
    echo Or use: fly proxy 10022:22 -a hearmeout-main
    echo Then:   scp -P 10022 youtube-cookies.txt root@localhost:/data/youtube-cookies.txt
)

echo.
echo Done! Restart the app: fly apps restart hearmeout-main
echo.
pause
